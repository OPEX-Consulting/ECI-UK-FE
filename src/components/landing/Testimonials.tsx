      ...
      <div className="container mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch max-w-6xl mx-auto">
          {testimonials.map((t, i) => (
            <div key={i} className="p-12 rounded-[32px] bg-white border border-border flex flex-col items-start relative hover:shadow-lg transition-all duration-300">
               {/* Quote Icon */}
               <div className="text-[448px] font-serif text-border/40 leading-none mb-4 h-8 select-none">
                 ”
               </div>
               
               <p className="text-[15px] md:text-[17px] font-serif text-foreground leading-relaxed mb-12">
                 "{t.quote}"
               </p>
               
               <div className="flex items-center gap-4 mt-auto">
                 <div className="w-10 h-10 rounded-full bg-[#1A3A2F] flex items-center justify-center text-[12px] font-bold text-white tracking-wider">
                   {t.initials}
                 </div>
                 <div className="flex flex-col">
                   <div className="text-[14px] font-bold text-foreground">
                     {t.author}, <span className="font-medium text-foreground/80">{t.role}</span>
                   </div>
