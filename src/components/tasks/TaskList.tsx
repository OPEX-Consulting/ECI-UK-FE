...
import { useTasks, Task, TaskStatus, TaskPriority, TaskRisk } from "@/contexts/TaskContext";
...
const TaskList = ({ onEditTask, tasks: propTasks }: TaskListProps) => {
  ...
  const getPriorityColor = (priority: TaskPriority) => {
    switch (priority) {
      ...
      case 'medium': return 'bg-yellow-100 text-yellow-800 hover:bg-yellow-100/80';
      case 'low': return 'bg-blue-100 text-blue-800 hover:bg-blue-100/80';
        ...
        <TableBody>
          {visibleTasks.length === 0 ? (
            ...
            visibleTasks.map((task) => (
              <TableRow key={task.id} className="cursor-pointer hover:bg-slate-50/50" onClick={() => onEditTask(task)}>
                <TableCell className="font-medium">
                    <div className="flex flex-col">
                        <span className="truncate max-w-[280px] font-semibold text-slate-900">{task.title}</span>
                ...
                <TableCell>
                    <Badge variant="outline" className={cn("capitalize font-normal border-0", getPriorityColor(task.priority))}>
                        {task.priority}
                    </Badge>
                </TableCell>
                <TableCell>
                     <div className="flex items-center gap-1.5">
                        <div className={cn("h-2 w-2 rounded-full bg-current", getRiskColor(task.risk))} />
                        <span className="capitalize text-slate-700">{task.risk}</span>
                     </div>
                </TableCell>
                <TableCell>
                                ...
                                <>
                                    ...
                                    {Object.entries(statusLabels).map(([key, label]) => (
                                        <DropdownMenuItem 
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                updateTask(task.id, { status: key as TaskStatus });
                                            }}
                                        >
                                            {label}
                                        </DropdownMenuItem>
