...
import { AlertCircle, Calendar, MoreHorizontal, CheckCircle2, Plus } from 'lucide-react';
    ...
    <div ref={setNodeRef} style={style} {...listeners} {...attributes} className={cn("touch-none", isDragging && "opacity-50 z-50")}>
        ...
        <CardContent className="p-4 space-y-3">
            ...
            <div className="flex gap-2">
                <Badge variant="outline" className={cn("capitalize px-2 py-0.5 rounded-sm font-normal text-xs", getPriorityColor(task.priority))}>
                    {task.priority} Priority
                </Badge>
                    {task.dueDate && new Date(task.dueDate) < new Date() && task.status !== 'done' && (
                        <Badge variant="destructive" className="px-2 py-0.5 rounded-sm font-normal text-xs flex items-center gap-1">
...
const DroppableColumn = ({ column, tasks, children }: { column: any; tasks: Task[]; children: React.ReactNode }) => {
  const { setNodeRef, isOver } = useDroppable({
    id: column.id,
  });

  return (
    <div key={column.id} className="flex-1 flex flex-col min-w-[280px] max-w-[350px] bg-slate-100/50 rounded-lg p-2 h-full">
        {/* Column Header */}
        <div className="flex items-center justify-between px-2 py-3 mb-2">
            ...
            <Badge variant="secondary" className="bg-slate-200 text-slate-600 px-2 h-5 text-[10px] min-w-[20px] justify-center">
                {tasks.length}
            </Badge>
            </div>
        </div>
        ...
        {/* Tasks List */}
        <div ref={setNodeRef} className={cn("flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3 min-h-[100px]", isOver && "bg-slate-200/50 rounded-lg transition-colors")}>
            {children}
            {tasks.length === 0 && (
...
const TaskBoard = ({ onEditTask }: TaskBoardProps) => {
  ...
  return (
    ...
    >
        ...
        <DragOverlay>
            {activeTask ? (
                 <Card className="mb-3 shadow-xl cursor-grabbing border-l-4 opacity-80 rotate-3" style={{ 
                    borderLeftColor: activeTask.risk === 'high' ? '#ef4444' : activeTask.risk === 'medium' ? '#f59e0b' : '#3b82f6',
                    width: '280px'
                }}>
                <CardContent className="p-4 space-y-3">
                    <div>
