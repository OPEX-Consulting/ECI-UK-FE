...
import { 
  ...
  Loader2, Search, Plus, Trash2, GripVertical,
  ChevronDown, ChevronUp, Save, Send, Globe
} from 'lucide-react';
import {
  Select,
  SelectContent,
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-[10px] uppercase font-bold tracking-widest text-primary">Compliance Intelligence</span>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-3xl mx-auto w-full mt-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden transition-colors duration-300">
              <div className="p-8 border-b border-border bg-muted/30">
                <h2 className="text-xl font-semibold mb-2">Upload source document</h2>
                <p className="text-sm text-muted-foreground">
                  Provide the underlying compliance document. Our AI will extract requirements and structure them automatically.
                </p>
              </div>
                ...
                <div className="min-h-[220px]">
                  ...
                  {uploadMode === 'url' && (
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Public Document URL</label>
                      ...
                      <p className="text-xs text-muted-foreground flex items-start gap-2">
                         ...
                         Ensure the URL is publicly accessible. Password-protected pages cannot be fetched.
                      </p>
                    </div>
                  )}
                  ...
                  {uploadMode === 'text' && (
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Paste Content</label>
                      <textarea
                        rows={8}
                      ...
                      <div className="flex justify-between mt-2">
                        <p className="text-[10px] text-muted-foreground">Min. 50 characters required</p>
                        <p className="text-[10px] text-muted-foreground">{rawText.length} characters</p>
                      </div>
                    </div>
                  )}
                </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-end mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium">Uploading and normalising...</span>
                        </div>
                        <span className="text-xs font-bold text-muted-foreground">{Math.round(uploadProgress)}%</span>
                      </div>
...
const StageEdit = ({ framework, setFramework, onNext, onSaveDraft, canPublish, sourceText }: { 
  ...
  const updateTheme = (themeId: string, updates: Partial<Theme>) => {
    setFramework({
      ...framework,
      themes: framework.themes.map(t => t.id === themeId ? { ...t, ...updates } : t)
    });
  };
  ...
  const addTask = (themeId: string) => {
    ...
    setFramework({
      ...framework,
      themes: framework.themes.map(t => t.id === themeId ? { ...t, tasks: [...t.tasks, newTask] } : t)
    });
  };
      ...
      <div className="w-full flex flex-col bg-card border border-border rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-border bg-muted/30 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Source document</span>
          </div>
...
const StageReview = ({ framework, onBack, onNext, canPublish }: { 
  ...
  const stats = {
    ...
    actionItems: framework.themes.reduce((acc, t) => acc + t.tasks.reduce((acc2, task) => acc2 + task.actionItems.length, 0), 0),
  };
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-muted/30 border border-border">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Themes</p>
              <p className="text-2xl font-bold">{stats.themes}</p>
            </div>
            <div className="p-4 rounded-xl bg-muted/30 border border-border">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Control Tasks</p>
              <p className="text-2xl font-bold">{stats.tasks}</p>
            </div>
            <div className="p-4 rounded-xl bg-muted/30 border border-border">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-1">Action Items</p>
              <p className="text-2xl font-bold">{stats.actionItems}</p>
            </div>
          </div>

          <div className="space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Framework Details</h3>
            <div className="p-6 rounded-xl border border-border bg-background/50 space-y-3">
              <p className="text-lg font-bold">{framework.name || "Untitled Framework"}</p>
              <p className="text-sm text-muted-foreground leading-relaxed">{framework.description || "No description provided."}</p>
            </div>
          </div>

          <div className="space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Governance Check</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold">Hierarchical integrity verified</p>
                  <p className="text-xs text-muted-foreground">All tasks are linked to valid themes and action items.</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">AI extraction was performed on 1 source. Human verification is required.</p>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full animate-in zoom-in-95 duration-700">
      ...
      <div className="mt-16 p-6 rounded-2xl bg-card border border-border flex items-center gap-4 max-w-sm">
        ...
        <div>
          ...
          <p className="text-sm text-foreground">Organisations can now subscribe to this framework and begin audits.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto w-full animate-in fade-in duration-700">
      <div className="text-center mb-12">
        ...
        <h2 className="text-2xl font-bold mb-3">AI is reading your document...</h2>
        <p className="text-muted-foreground">
          This usually takes 20–60 seconds. We are structuring the requirements into themes and tasks.
        </p>
      </div>
        ...
        {hasError && (
          <div className="flex items-center justify-between p-4 rounded-xl bg-red-500/5 border border-red-500/10 animate-in slide-in-from-top-2">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium text-red-500">Extraction failed. Please try again.</p>
            </div>
            <button 
              onClick={() => window.location.reload()}
